  ...
  switch( mergetype ){
    ...
    default: assert( mergetype==MERGE_POS_NEAR || mergetype==MERGE_NEAR ); {
      ...
      if( mergetype==MERGE_POS_NEAR ){
        ppPos = &p;
        aTmp = (char *) sqlite3_malloc(2*(n1+n2+1));
        if( !aTmp ){
          return SQLITE_NOMEM;
        }
      }
  ...
  for(i=0; i<SizeofArray(pTS->aaOutput); i++){
    if( pTS->aaOutput[i] ){
      if( !aOut ){
        ...
        char *aNew = (char *) sqlite3_malloc(nNew);
        if( !aNew ){
          sqlite3_free(aOut);
          return SQLITE_NOMEM;
        }
...
static int fts3TermSelectCb(
  ...
  if( pTS->aaOutput[0]==0 ){
    ...
    */
    pTS->aaOutput[0] = (char *) sqlite3_malloc(nDoclist);
...
static int fts3DeferredTermSelect(
  ...
  if( !aSource ){
    ...
    *ppOut = (char *) sqlite3_malloc(nSource);
    ...
    *ppOut = (char *) sqlite3_malloc(*pnOut);
    if( !*ppOut ) return SQLITE_NOMEM;
...
SQLITE_PRIVATE int sqlite3Fts3SegReaderCursor(
  ...
  if( iLevel!=FTS3_SEGCURSOR_PENDING ){
    ...
    while( rc==SQLITE_OK && SQLITE_ROW==(rc = sqlite3_step(pStmt)) ){
      ...
      char const *zRoot = (const char *) sqlite3_column_blob(pStmt, 4);
...
static int fts3TermSegReaderCursor(
  ...
  pSegcsr = (Fts3SegReaderCursor *) sqlite3_malloc(sizeof(Fts3SegReaderCursor));
  if( pSegcsr ){
...
static int fts3NearMerge(
  ...
  aOut = (char *) sqlite3_malloc(nLeft+nRight+1);
  if( aOut==0 ){
    rc = SQLITE_NOMEM;
  }else{
  ...
  *pnOut = 0;
  ...
  if( pExpr ){
    ...
    if( pExpr->eType==FTSQUERY_PHRASE ){
      ...
      if( rc==SQLITE_OK ){
        assert( nExpr>1 );
        aExpr = (ExprAndCost *) sqlite3_malloc(sizeof(ExprAndCost) * nExpr);
       ...
       && 0==(rc = fts3EvalExpr(p, pExpr->pLeft, &aLeft, &nLeft, isReqPos))
        ...
        switch( pExpr->eType ){
          ...
          case FTSQUERY_OR: {
            ...
            */
            char *aBuffer = (char *) sqlite3_malloc(nRight+nLeft+1);
...
SQLITE_PRIVATE int sqlite3Fts3Init(sqlite3 *db){
  ...
  pHash = (Fts3Hash *) sqlite3_malloc(sizeof(Fts3Hash));
  if( !pHash ){
    rc = SQLITE_NOMEM;
  }else{
